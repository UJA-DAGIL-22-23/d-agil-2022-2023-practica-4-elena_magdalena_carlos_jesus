/**
 * @file callbacks.js
 * @description Callbacks para el MS PlantillaCurling.
...
const CB_OTHERS = {
    ...
    home: async (req, res) => {
        try {
            CORS(res).status(200).json({ mensaje: "Microservicio MS PlantillaCurling: home" });
        } catch (error) {
            CORS(res).status(500).json({ error: error.description })
        }
    },
    ...
    acercaDe: async (req, res) => {
        try {
            CORS(res).status(200).json({
                mensaje: "Microservicio MS PlantillaCurling: acerca de",
