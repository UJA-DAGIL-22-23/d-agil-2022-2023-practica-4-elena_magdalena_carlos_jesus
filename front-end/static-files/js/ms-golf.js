/**
 * @file Golf.js
 * @description Funciones para el procesamiento de la info enviada por el MS Golf
 * @author Jesús Manzano <jma00068@red.ujaen.es>
 */
...
Golf.tablaJugadores.cabecera = `<table width="100%" class="listado-personas">
                    <thead>
                        <th width="15%">Id <a href="javascript:Golf.procesarJugadores(${ORDENAR_ID})" class="opcion-secundaria mostrar">V</a> </th>
                        <th width="10%">Nombre <a href="javascript:Golf.procesarJugadores(${ORDENAR_NOMBRE})" class="opcion-secundaria mostrar">V</a> </th>
                        <th width="15%">Apellidos <a href="javascript:Golf.procesarJugadores(${ORDENAR_APELLIDOS})" class="opcion-secundaria mostrar">V</a> </th>
                        <th width="15%">Fecha Nacimiento <a href="javascript:Golf.procesarJugadores(${ORDENAR_FECHA_NAC})" class="opcion-secundaria mostrar">V</a> </th>
                        <th width="20%">Años gana premios <a href="javascript:Golf.procesarJugadores(${ORDENAR_ANIOS_GANA_PREMIOS})" class="opcion-secundaria mostrar">V</a> </th>
                        <th width="15%">Número campeonatos <a href="javascript:Golf.procesarJugadores(${ORDENAR_NUM_CAMPEONATOS})" class="opcion-secundaria mostrar">V</a> </th>
...
Golf.tablaJugadores.cuerpo = `
    <tr title="${Golf.plantillaTags.ID}">
        <td>${Golf.plantillaTags.ID}</td>
        <td>${Golf.plantillaTags.NOMBRE}</td>
        <td>${Golf.plantillaTags.APELLIDOS}</td>
        <td>${Golf.plantillaTags["FECHA NACIMIENTO"]}</td>
        <td>${Golf.plantillaTags["AÑOS GANA PREMIOS"]}</td>
        <td>${Golf.plantillaTags["NÚMERO CAMPEONATOS"]}</td>
        <td>
                    <div><a href="javascript:Golf.mostrar('${Golf.plantillaTags.ID}')" class="opcion-secundaria mostrar">Mostrar</a></div>
        </td>
    </tr>
    `;

// Pie de la tabla
Golf.tablaJugadores.pie = `        </tbody>
             </table>
             `;

/// Formulario para añadir un nuevo jugador
Golf.formularioAniadeJugador = `<form method='post' action='' id='formularioNuevoJugador'>
    <table width="100%" class="listado-personas">
        ...
        <tbody>
            <tr>
                ...
                <td>
                    <div><a href="javascript:Golf.aniadirJugador()" class="opcion-secundaria mostrar">Añadir</a></div>
                </td>
            </tr>
        </tbody>
    </table>
 ...
 */           
Golf.sustituyeTags = function (plantilla, jugador) {
    let fecha_nac = jugador.data.fecha_nac.dia+"/"+jugador.data.fecha_nac.mes+"/"+jugador.data.fecha_nac.anio
    return plantilla
        .replace(new RegExp(Golf.plantillaTags.ID, 'g'), jugador.ref['@ref'].id)
        .replace(new RegExp(Golf.plantillaTags.NOMBRE, 'g'), jugador.data.nombre)
        .replace(new RegExp(Golf.plantillaTags.APELLIDOS, 'g'), jugador.data.apellidos)
        .replace(new RegExp(Golf.plantillaTags["FECHA NACIMIENTO"], 'g'), fecha_nac)
        .replace(new RegExp(Golf.plantillaTags["AÑOS GANA PREMIOS"], 'g'), jugador.data.anio_gana_premio)
        .replace(new RegExp(Golf.plantillaTags["NÚMERO CAMPEONATOS"], 'g'), jugador.data.num_campeonatos)
}
 ...
 * @returns La plantilla del cuerpo de la tabla con los datos actualizados 
 */
Golf.tablaJugadores.actualiza = function (jugador) {
    return Golf.sustituyeTags(this.cuerpo, jugador)
}
 ...
 */
Golf.descargarRuta = async function (ruta, callBackFn, opcion) {
    let response = null
 ...
 */
Golf.mostrarHome = function (datosDescargados) {
    ...
    Frontend.Article.actualizar("Golf Home", datosDescargados.mensaje)
}

/**
 * Función principal para mostrar los datos enviados por la ruta "acerca de" de MS Plantilla
 */
Golf.mostrarAcercaDe = function (datosDescargados) {
    // Si no se ha proporcionado valor para datosDescargados
    datosDescargados = datosDescargados || this.datosDescargadosNulos
    ...
    `;
    Frontend.Article.actualizar("Golf Acerca de", mensajeAMostrar)
}

/**
 * Función principal para mostrar los datos enviados por la ruta "getNombres" de MS Plantilla
 */
Golf.mostrarNombres = function (datosDescargados) {
    ...
    // Muestro todos los jugadores que se han descargado
        ...
        let numJugador = 1
        mensajeAMostrar = `<a href="javascript:Golf.procesarNombresOrdenados()" class="opcion-principal"
        title="Llama a la ruta /getNombres del MS Golf">Ordenar alfabéticamente</a><ul>`;
 ...
 */
Golf.mostrarNombresOrdenados = function (datosDescargados) {
    // Mensaje que se enviará para mostrar los datos
    let mensajeAMostrar = ""
    ...
    // Muestro todos los jugadores que se han descargado de forma ordenada alfabéticamente
        ...
        let numJugador = 1
        mensajeAMostrar = `<a href="javascript:Golf.procesarNombres()" class="opcion-principal"
        title="Llama a la ruta /getNombres del MS Golf">Quitar orden alfabético</a><ul>`;
        datosDescargados.data.forEach(element => {
            mensajeAMostrar += `<li><b>Nombre jugador ${numJugador}</b>: ${element}</li>`
            numJugador++
        });
...
Golf.imprimeMuchosJugadores = function (datosDescargados, opOrdenar) {
    // Mensaje que se enviará para mostrar los datos
    let mensajeAMostrar = ""
    ...
    // Muestro todos los jugadores que se han descargado en forma de tabla
        ...
        Golf.ordenaJugadores(vectorJugadores, opOrdenar) // Ordenamos los datos en caso de que sea necesario

        // Compongo el contenido que se va a mostrar dentro de la tabla
        mensajeAMostrar = Golf.tablaJugadores.cabecera
        vectorJugadores.forEach(e => mensajeAMostrar += Golf.tablaJugadores.actualiza(e))
        mensajeAMostrar += Golf.tablaJugadores.pie
 ...
 */
Golf.ordenaJugadores = function (vectorJugadores, opOrdenar) {
    switch (opOrdenar) {
        case ORDENAR_ID:
...
Golf.imprimeUnJugador = function (datosDescargados) {
    // Mensaje que se enviará para mostrar los datos
    let mensajeAMostrar = ""
            ...
            <tbody>
                <tr title="${datosDescargados.ref['@ref'].id}">
                    ...
                    <td><input type="number" class="form-persona-elemento editable" disabled
                            ...
                            name="numcampeonatos_jugador"/></td>
                    <td>
                        <div><a href="javascript:Golf.editar()" class="opcion-secundaria mostrar">Editar</a></div>
                        <div><a href="javascript:Golf.eliminar()" class="opcion-secundaria mostrar">Eliminar</a></div>
                        <div><a href="javascript:Golf.guardar()" class="opcion-terciaria editar ocultar">Guardar</a></div>
                        <div><a href="javascript:Golf.mostrar('${datosDescargados.ref['@ref'].id}')" class="opcion-terciaria editar ocultar">Cancelar</a></div>
                    </td>
                </tr>
            </tbody>
        </table>
 ...
 */
Golf.opcionesMostrarOcultar = function (classname, mostrando) {
 ...
 */
Golf.habilitarDeshabilitarCamposEditables = function (deshabilitando) {
    deshabilitando = (typeof deshabilitando === "undefined" || deshabilitando === null) ? true : deshabilitando
    for (let campo in Golf.form) {
        document.getElementById(Golf.form[campo]).disabled = deshabilitando
    }
    return this
}
...
Golf.editar = function () {
    ...
    Golf.habilitarDeshabilitarCamposEditables(false) // Habilita los campos editables
}
 ...
 */
Golf.aniadeJugador = function () {
    // Borro toda la info de Article y la sustituyo por la que me interesa
    Frontend.Article.actualizar("Añadir un jugador", Golf.formularioAniadeJugador)
}
